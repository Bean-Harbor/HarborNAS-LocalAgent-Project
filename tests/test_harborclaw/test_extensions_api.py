"""Tests for harborclaw.api.extensions_api (ExtensionStore + HarborClawExtensionsAPI)."""
...
from harborclaw.api.extensions_api import (
    ExtensionStore,
    HarborClawExtensionsAPI,
    ValidationResultDTO,
...
@pytest.fixture()
def api(store: ExtensionStore) -> HarborClawExtensionsAPI:
    return HarborClawExtensionsAPI(store=store)
...
class TestExtensionsAPI:
    ...
    def test_list_empty(self, api: HarborClawExtensionsAPI) -> None:
        result = api.list_extensions()
        assert result == []

    def test_create_and_list(self, api: HarborClawExtensionsAPI) -> None:
        api.create_extension(_sample_manifest())
        items = api.list_extensions()
        assert len(items) == 1
        assert items[0]["id"] == "test.ext"

    def test_get_extension(self, api: HarborClawExtensionsAPI) -> None:
        api.create_extension(_sample_manifest())
        detail = api.get_extension("test.ext")
        assert detail["name"] == "Test Extension"

    def test_update_extension(self, api: HarborClawExtensionsAPI) -> None:
        api.create_extension(_sample_manifest())
    ...
    def test_delete_extension(self, api: HarborClawExtensionsAPI) -> None:
        api.create_extension(_sample_manifest())
        api.delete_extension("test.ext")
        assert api.list_extensions() == []

    def test_validate_extension(self, api: HarborClawExtensionsAPI) -> None:
        result = api.validate_extension(_sample_manifest())
        assert result["valid"] is True

    def test_validate_bad_extension(self, api: HarborClawExtensionsAPI) -> None:
