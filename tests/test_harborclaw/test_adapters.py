"""Tests for harborclaw.adapters — IM channel adapters."""
import json
import pytest

from harborclaw.adapters import (
    ChannelAdapter,
    DingTalkAdapter,
    DiscordAdapter,
    FeishuAdapter,
...
class TestTelegramAdapter:
    ...
    def test_strips_command_prefix(self):
        raw = {"message": {"from": {"id": 1}, "text": "/harborclaw status plex"}}
        msg = self.adapter.parse_inbound(raw)
...
class TestMqttAdapter:
    ...
    def test_build_outbound(self):
        out = OutboundMessage(
            ...
            recipient_id="c1",
            text="ok",
            payload={"topic": "harborclaw/reply/c1"},
        )
        payload = self.adapter.build_outbound(out)
        assert payload["topic"] == "harborclaw/reply/c1"
