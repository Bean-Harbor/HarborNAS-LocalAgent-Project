"""Generate HarborBeacon TOML tool descriptions from skill manifests.

HarborBeacon discovers tools via ``tool_descriptions/<lang>.toml`` files.
This module converts our SkillManifest capabilities into that format,
enabling HarborBeacon to display localised tool descriptions to users.
"""
...
def registry_to_toml(registry: Registry, *, lang: str = "en") -> str:
    ...
    Compatible with HarborBeacon's ``tool_descriptions/<lang>.toml`` format.
    """
...
def manifest_to_skill_toml(manifest: SkillManifest) -> str:
    """Generate a HarborBeacon SKILL.toml for a single skill manifest.

    Skills live at ``/etc/harborbeacon/skills/<name>/SKILL.toml``.
    """
